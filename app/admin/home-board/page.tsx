...
import { HomeBoardManager } from "@/components/admin/home-board-manager"
...
export default async function HomeBoardAdminPage() {
  const { userId } = await auth()
      ...
      <div className="admin-gate-screen">
        <Card className="w-full max-w-md">
          ...
          <CardContent className="flex justify-center">
            <Button asChild>
              <Link href="/sign-in?redirect_url=/admin/home-board">Sign In</Link>
            </Button>
          </CardContent>
        </Card>
      </div>
  ...
  return (
    <div className="admin-shell">
      <AdminNav currentPage="home-board" admin={admin} />
      <main id="main-content" className="admin-main">
        <div className="admin-container admin-container--narrow">
          <header className="admin-page-header">
            <h1 className="text-section-title text-balance">Home board</h1>
            <p className="text-muted-foreground mt-1">
              Control what families see on the app Today tab — order, visibility, and live banners.
            </p>
          </header>
          <HomeBoardManager canManage={canManage} />
        </div>
      </main>
    </div>
  )
