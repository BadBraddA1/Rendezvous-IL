...
- **Worship service volunteers** (`/admin/volunteers`, nav: Event → Volunteers) — manages the "Volunteer for Worship Services" signups collected during registration (`volunteer_signups`). An **event year picker** (same `?year=` + sessionStorage pattern as Registration Management, shared storage key so the year follows you between admin pages) filters everything by the registration's `event_year` and switches the schedule editor's service dates (Mondays per year in `EVENT_MONDAY`, `components/admin/volunteer-schedule-manager.tsx`). Two tabs: **Schedule** (pick one of the 8 devotion services for the selected year, e.g. Mon 5/3 PM – Fri 5/7 AM for 2027, and fill its 8 slots — Opening/Closing Prayer, A/B Leading singing, A/B Reading scripture, A/B Presenting a lesson with title + scripture fields — plus a roster to search/filter all signups, see or clear assignments, jump to the family's registration) and **Lesson bids** (below). Slots can be filled three ways: the **dropdown**, **drag-and-drop** (drag a chip from the "Available for [service]" tray onto a slot — slots only accept matching roles), or **Auto-fill week** (greedy fill of every empty slot across the week that balances load and prefers topic-winners for lesson slots; shows a **draft preview grouped by service that saves nothing until "Approve & apply"**, which posts to `POST /api/admin/volunteers/bulk-assign`). **Booking rules** (enforced in the UI and again server-side in `lib/volunteer-scheduling.ts`, returning 409): one volunteer per slot, and one booking per person per day — a person = registration + name, so someone signed up for several roles still gets at most one slot per day and each role at most once. Assigning writes `assigned_date` / `time_slot` / `prayer_type` on the signup, which the existing public schedule (`GET /api/volunteer-schedule`, `/schedule` page, Live Updates) reads directly — no schema change. API: `GET /api/admin/volunteers`, `PATCH /api/admin/volunteers/[id]`, `POST /api/admin/volunteers/bulk-assign` (all audit-logged; editing requires admin/editor role, viewers are read-only).
- **Lesson bid system** (`/admin/volunteers` → Lesson bids tab) — rebuilt from the old admin dash on the surviving `lesson_topics` + `lesson_bids` tables (created lazily by `lib/lesson-bids.ts`). Topics are **per event year** (`lesson_topics.event_year`, added lazily, NULL = 2027) and the tab follows the page's year picker. Admins maintain the **topics list** (add/edit/delete; claimed topics can't be deleted) and see every "Presenting a lesson" volunteer with a status chain: Not invited → Invited → Bid submitted (ranked picks shown) → Awarded. **Send bid invite** emails the presenter a personal `/lesson-bid/[token]` link (Resend; recipient = the family member whose first name matches the volunteer and has an email, else the registration's primary email; token stored on `volunteer_signups.lesson_bid_token`). The public page lets them rank up to 3 open topics and resubmit until a topic is awarded; **after awarding, the same link stays live** so the presenter can enter their lesson title and scripture reading (`volunteer_signups.lesson_title` / `scripture_reading`), which **override the raw topic title** on the public schedule (`COALESCE(NULLIF(vs.lesson_title,''), lt.title)`). **Awarding is an admin decision** — the dropdown stars the presenter's picks (★ #1/#2/#3) but any open topic can be chosen; un-award supported; all actions audit-logged. Awarding sets `volunteer_signups.claimed_lesson_id` + `lesson_topics.claimed_*`, so the topic title shows on `/schedule` and Live Updates immediately (queries now `COALESCE` the claimed topic's title/description with the manual `lesson_title` / `scripture_reading` fields, which fixed manual entries never displaying). The Schedule tab's lesson slots show a read-only "Awarded topic" hint instead of the manual fields when a topic is claimed. API: `GET/POST /api/admin/lesson-topics`, `PATCH/DELETE /api/admin/lesson-topics/[id]`, `POST /api/admin/volunteers/[id]/lesson-bid` (`invite` / `award` / `unaward`), public `POST /api/lesson-bid/[token]`.
