...
Brand and UI context: `PRODUCT.md`, `DESIGN.md`. Marketing surfaces polished: lake-teal OKLCH palette, Libre Baskerville + Libre Franklin, mobile safe-areas, accessible focus rings, semantic success/warning tokens. **Mux video:** `components/mux-video-player.tsx` — one-tap iOS playback, captions off by default (`defaultHiddenCaptions`); FAQ explains iOS Settings override; FAQ uses `deferPlayer` for lazy mount. **Social sharing:** dynamic `app/opengraph-image.tsx` + `app/twitter-image.tsx` (1200×630, group photo + logo). **Homepage:** animated color-wave “2027” hero, split-column “What to expect”, planning-2027 dates hero + chips, hero Mux video. **`/faq`:** accordion + “Around the campfire” organizer videos. **`/schedule`:** draft-notice callout, tokenized print view, extra mobile header clearance. **`/live-updates`:** lake-teal display board with flat `.lu-panel` surfaces (no glass/violet gradients). shadcn `Button` defaults to `min-h-11` for touch targets.
