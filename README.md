...
- **Worship service volunteers** (`/admin/volunteers`, nav: Event → Volunteers) — manages the "Volunteer for Worship Services" signups collected during registration (`volunteer_signups`). Two sections: a **schedule editor** (pick one of the 8 devotion services, Mon 5/3 PM – Fri 5/7 AM, and fill its 8 slots — Opening/Closing Prayer, A/B Leading singing, A/B Reading scripture, A/B Presenting a lesson with title + scripture fields) and a **roster** (search/filter all signups, see or clear assignments, jump to the family's registration). Assigning writes `assigned_date` / `time_slot` / `prayer_type` on the signup, which the existing public schedule (`GET /api/volunteer-schedule`, `/schedule` page, Live Updates) reads directly — no schema change. API: `GET /api/admin/volunteers`, `PATCH /api/admin/volunteers/[id]` (audit-logged; editing requires admin/editor role, viewers are read-only).
