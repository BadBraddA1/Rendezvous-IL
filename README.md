...
Brand and UI context: `PRODUCT.md`, `DESIGN.md`. Marketing surfaces polished: lake-teal OKLCH palette, Libre Baskerville + Libre Franklin, mobile safe-areas, accessible focus rings, semantic success/warning tokens. **Mux video:** `components/mux-video-player.tsx` — one-tap iOS playback, captions off by default (`defaultHiddenCaptions`), FAQ uses `deferPlayer` so only the active clip mounts a player. **Homepage:** animated color-wave “2027” hero, split-column “What to expect”, planning-2027 dates hero + chips, hero Mux video. **`/faq`:** accordion + “Around the campfire” organizer videos. **`/schedule`:** draft-notice callout (full border, no side-stripe), print view uses design tokens, extra mobile header clearance. **`/live-updates`:** display board uses lake-teal dark shell; violet/glass/blur reduced. Site header Sign In meets 44px touch target.
