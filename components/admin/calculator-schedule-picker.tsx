...
export function CalculatorSchedulePicker({
  ...
  const schedule = resolveMemberSchedule(attendance)
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1 min-w-0">
          <Label className="text-xs text-muted-foreground">
            {driveIn ? "Meals to add" : "Nights & meals"}
          </Label>
          <p className="text-sm font-medium text-balance">{schedule.label}</p>
          <p className="text-xs text-muted-foreground">{schedule.pricingNote}</p>
        </div>
        <Badge variant="secondary" className="w-fit shrink-0 tabular-nums">
          {driveIn ? `${mealCount} meals` : `${attendance.nights.length} nights · ${mealCount} meals`}
        </Badge>
      </div>
